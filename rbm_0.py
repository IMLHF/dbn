...
class RBM(object):
  def __init__(self, name, isize, osize, learning_rate=0.01, CDk=1):
    ...
    self._weight = tf.Variable(tf.zeros([isize,osize]))
    self._v_bias = tf.Variable(tf.zeros([isize]))
    self._h_bias = tf.Variable(tf.zeros([osize]))
  ...
  def get_param(self):
    with tf.Session() as sess:
      init = tf.global_variables_initializer()
      sess.run(init)
      return sess.run(self._weight),sess.run(self._v_bias),sess.run(self._h_bias)
